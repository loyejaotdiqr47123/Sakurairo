    ...
    <div class="footer-device function_area">
      ...
      <span style="color: #b9b9b9;">
        ...
        <?php if (iro_opt('footer_load_occupancy', 'true')): ?>
          ...
          $load = sys_getloadavg();
          // 获取当前时间（以秒为单位）
          $loadTime = timer_stop(0, 3);
          // 获取查询数量
          $numQueries = get_num_queries();
          // 获取内存峰值使用量（以 MB 为单位）
          $memoryUsage = memory_get_peak_usage() / 1024 / 1024;
          // 获取 CPU 使用率（假设以 1 分钟的平均负载）
          $cpuUsage = $load[0];
          ?>

          <?php printf(
            _x('加载时间 %.3f 秒 | 查询次数 %d | CPU 使用率 %.2f | 内存使用 %.2f MB ', 'footer load occupancy', 'sakurairo'),
            $loadTime,
            $numQueries,
            $cpuUsage,
            $memoryUsage
...
<?php if (iro_opt('wave_effects', 'true')): ?>
  <link rel="stylesheet" href="<?php global $shared_lib_basepath;
                                echo $shared_lib_basepath ?>/css/wave.css">
<?php endif; ?>
